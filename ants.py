...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        curr = 0
        while curr != self.min_range and ant_place is not beehive:
            ant_place = ant_place.entrance
            curr += 1
        while (self.min_range <= curr <= self.max_range) and ant_place is not beehive:
            if ant_place.bees:
...
class ScubaThrower(ThrowerAnt):
    """A watersafe ThrowerAnt. Same as ThrowerAnt, just survives in water."""
    ...
    def action(self, gamestate):
        ...
        if self.impostor:
            ...
            while ant_place is not None:
                ant = ant_place.ant
                if ant is not None and not ant.doubled_up:
                    ant.damage *= 2
                    ant.doubled_up = True
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self, beehive):
        ...
        while ant_place is not beehive:
            if ant_place.ant is not None and ant_place.ant is not self:
                iif[ant_place.ant] = dist
